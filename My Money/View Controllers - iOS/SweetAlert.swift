...
class SweetAlert: UIViewController {
	...
	func resizeAndRelayout() {
		...
		for i in 0 ..< buttons.count {
			...
			buttons[i].addTarget(self, action: #selector(pressed(_:)), for: UIControlEvents.touchUpInside)
		}
	...
	func showAlert(_ title: String) -> SweetAlert {
		let _ = self.showAlert(title, subTitle: nil, style: .none)
		return self
	}

	func showAlert(_ title: String, subTitle: String?, style: AlertStyle) -> SweetAlert {
		let _ = self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: "OK")
		return self
	}

	func showAlert(_ title: String, subTitle: String?, style: AlertStyle, buttonTitle: String, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
		let _ = self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle, buttonColor: UIColorFromRGB(0xAEDEF4))
		userAction = action
		return self
	}

	func showAlert(_ title: String, subTitle: String?, style: AlertStyle, buttonTitle: String, buttonColor: UIColor, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
	...
	func showAlert(_ title: String, subTitle: String?, style: AlertStyle, buttonTitle: String, buttonColor: UIColor, otherButtonTitle:
	...
	func showAlert(_ title: String, subTitle: String?, style: AlertStyle, buttonTitle: String, buttonColor: UIColor, otherButtonTitle:
			String?, otherButtonColor: UIColor?, action: ((_ isOtherButton: Bool) -> Void)? = nil) {
				userAction = action
				let window = UIApplication.shared.keyWindow?.subviews.first!
				...
				if otherButtonTitle != nil && otherButtonTitle!.isEmpty == false {
					...
					button.addTarget(self, action: #selector(pressed(_:)), for: UIControlEvents.touchUpInside)
					button.tag = 1
					buttons.append(button)
				}
